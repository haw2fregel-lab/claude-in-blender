...
- **Forked sessions.** The panel grows a copy of your desktop Claude Code session —
  conversation context, your settings and your `CLAUDE.md` carry over, and the original
  conversation is never written to.
...
- **A traceable log.** Every line of Python that Claude runs is appended to a
  `claude_bridge_log` Text datablock, so you can read afterwards exactly what happened.
...
- No credentials are read, stored or transmitted. Authentication and usage ride on your
  already-logged-in Claude Code account.
