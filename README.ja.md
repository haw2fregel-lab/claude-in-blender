...
パネル最上段の **Work dir** が、依頼を動かす作業ディレクトリです。**このリポである必要はありません**——自分のプロジェクトを指定すれば、そのリポの `CLAUDE.md` やスキルがそのまま効きます。追加するには、このリポで次を実行してください。

```bash
python tools/bridge_register.py --cwd /path/to/your/project
```

一度使ったディレクトリはドロップダウンに残ります（最新 5 件）。切り替えると今の会話は外れるため、接続中は選べません——先に切断してから選んでください。
