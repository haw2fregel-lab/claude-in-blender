...
- Text エディタの **`claude_bridge_log`** には、送信したコード・result の先頭200文字・エラー抜粋が残る。stdout/stderr は取り込まない。5,000行を超えると古い側を切り詰め、直近約2,500行を残す
...
- **ログは `.blend` の中に保存されます**——`claude_bridge_log` は送信コードと result・エラー抜粋を含んだまま、ファイルと一緒に付いていきます。人に渡す前に消してください
